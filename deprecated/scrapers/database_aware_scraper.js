...
// Extract variant ID from 99spokes URL
const extractVariantIdFromUrl = (url) => {
  if (!url) return null;
...
// Extract variant name (trim/groupset) from comprehensive data
const extractVariantFromData = (comprehensiveData) => {
  if (!comprehensiveData) return null;
  
  // Try to extract groupset/variant info from different locations
  const sources = [
...
const saveScrapedDataToDatabase = async (variantId, comprehensiveData) => {
  try {
    ...
    // Extract variant info from comprehensive data
    const variantName = extractVariantFromData(comprehensiveData) || 'Standard';
    ...
    if (catalogResult.rows.length > 0) {
      ...
      console.log(`💾 Updated ${variantId} (keyid: ${keyid}) in database`);
    } else {
      ...
      console.log(`💾 Inserted ${variantId} (keyid: ${keyid}) into database`);
    }
...
const checkVariantExistsInDatabase = async (variantId) => {
  try {
    // This function is less useful now since we don't query by variantId directly
...
for (const [key, data] of Object.entries(bikeVariants)) {
  for (const family of data.families || []) {
    for (const variant of family.variants || []) {
      ...
      if (existingSpec) {
        matchedCount++;
        if (uniqueMatches.has(variant.variantId)) {
          // Track duplicates
          if (!duplicateExamples[variant.variantId]) {
            duplicateExamples[variant.variantId] = [];
          }
          duplicateExamples[variant.variantId].push(`${key}/${family.familyId}`);
        }
...
const duplicateKeys = Object.keys(duplicateExamples);
if (duplicateKeys.length > 0) {
  console.log(`\n🔍 DUPLICATE ANALYSIS:`);
  console.log(`   Total duplicates: ${duplicateKeys.length} variant IDs appear multiple times`);
  console.log(`   Example duplicates:`);
  duplicateKeys.slice(0, 3).forEach(variantId => {
    console.log(`     ${variantId}: appears in ${duplicateExamples[variantId].slice(0, 3).join(', ')}`);
  });
}
