...
function setupEventListeners() {
    // Type selection
    document.querySelectorAll('.add-bikes-type-card').forEach(card => {
    ...
    document.getElementById('v2-breadcrumb').addEventListener('click', (e) => {
        if (e.target.classList.contains('add-bikes-breadcrumb-item') && !e.target.classList.contains('active')) {
    ...
    // View toggle
    document.querySelectorAll('.add-bikes-view-btn').forEach(btn => {
        btn.addEventListener('click', () => toggleView(btn.dataset.view));
    });
    
    // Global search
    document.querySelector('.add-bikes-global-search-input')?.addEventListener('input', (e) => {
