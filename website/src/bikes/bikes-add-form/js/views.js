...
export function renderBrands(brands, type) {
    ...
    Object.keys(grouped).sort().forEach(letter => {
        ...
        separator.className = 'add-bike-brand-letter-separator';
        ...
        grouped[letter].forEach(brand => {
            ...
            brandItem.className = 'add-bike-brand-item';
            const icons = {
            ...
            brandItem.innerHTML = `
                <div class="add-bike-brand-icon">${icons[type] || icons['bicycle']}</div>
                <div class="add-bike-brand-name">${brand.name}</div>
                ${brand.count ? `<div class="add-bike-brand-count">${brand.count} models</div>` : ''}
            `;
...
export function renderYears(years) {
    ...
    Object.keys(decades).sort((a, b) => b - a).forEach(decade => {
        ...
        decadeSection.className = 'add-bike-decade-section';
        decadeSection.innerHTML = `<h3>${decade}s</h3>`;
        
        const yearGrid = document.createElement('div');
        yearGrid.className = 'add-bike-year-grid';
        
        decades[decade].sort((a, b) => b - a).forEach(year => {
            const yearItem = document.createElement('div');
            yearItem.className = 'add-bike-year-item';
...
export function renderModels(modelData, type) {
    ...
    models.forEach(model => {
        ...
        modelCard.className = 'add-bike-model-card';
...
export function renderVariants(variants, type) {
    ...
    if (type === 'bicycle') {
        ...
        variants.forEach(variant => {
            ...
            variantItem.className = 'add-bike-variant-item';
            variantItem.innerHTML = `
                <div class="add-bike-variant-icon"><svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="18" r="3"></circle><circle cx="12" cy="6" r="3"></circle><path d="M12 9v6"></path></svg></div>
                <div class="add-bike-variant-info">
                    <h4>${variant}</h4>
                    <p>View specifications</p>
                </div>
                <div class="add-bike-variant-arrow"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 18 15 12 9 6"></polyline></svg></div>
            `;
        ...
        variants.forEach(variant => {
            ...
            variantItem.className = 'add-bike-variant-item';
            variantItem.innerHTML = `
                <div class="add-bike-variant-icon"><svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="18" cy="18" r="3"></circle><circle cx="6" cy="18" r="3"></circle><path d="M6 15h12v-4l-3-3h-6l-3 3v4z"></path></svg></div>
                <div class="add-bike-variant-info">
