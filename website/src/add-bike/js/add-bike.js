...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function loadMotorcycleYears(make) {
        try {
            const response = await fetch(`http://localhost:8080/api/motorcycles/years/${encodeURIComponent(make)}`);
    ...
    async function loadMotorcycleModels(make, year) {
        try {
            const response = await fetch(`http://localhost:8080/api/motorcycles/models/${encodeURIComponent(make)}/${year}`);
