...
// Load batch_5_brands.js
const batch5Path = path.join(__dirname, 'batch_5_brands.js');
const batch5Content = fs.readFileSync(batch5Path, 'utf8');
const batch5Match = batch5Content.match(/const batch_5_brands = (\[[\s\S]*\]);/);
const batch5Brands = eval(batch5Match[1]);
...
const newBrands = batch5Brands.filter(brand => !existingBrandIds.has(brand.brand_id));

console.log(`Found ${batch5Brands.length} brands in batch_5_brands.js`);
