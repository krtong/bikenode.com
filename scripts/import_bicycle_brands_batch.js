...
/**
 * Clean JSON content by removing comments
 */
...
/**
 * Main function to import bicycle brands from file
 */
async function importBicycleBrands() {
  const pool = new pg.Pool(dbConfig);
  
  try {
    console.log(chalk.blue('🚀 Starting bicycle brands import...'));
    ...
    // Clean the JSON content (remove comments)
    ...
    console.log(chalk.blue(`📊 Found ${brandsData.length} brand(s) to import`));
    ...
    for (const brandData of brandsData) {
      try {
        ...
        console.log(chalk.green(`✅ Imported: ${brandData.brand_name} (${brandData.brand_id})`));
        successCount++;
      } catch (error) {
        console.error(chalk.red(`❌ Failed to import: ${brandData.brand_name || 'Unknown'} - ${error.message}`));
        errorCount++;
      }
    }
    ...
    console.log(chalk.green(`✅ Successfully imported: ${successCount} brands`));
    if (errorCount > 0) {
      console.log(chalk.red(`❌ Failed to import: ${errorCount} brands`));
    }
