...
if MINIMAL_MODE:
    logger.info("Running in MINIMAL MODE - only Claude commands will be loaded")
...
async def setup_bot():
    ...
    # Always load Claude commands
    await bot.add_cog(ClaudeFixed(bot))
    
    if not MINIMAL_MODE:
        # Load all other command and event cogs
    ...
    logger.info(f"All cogs loaded successfully ({'minimal mode' if MINIMAL_MODE else 'full mode'})")
