...
class EnhancedBikeScraper {
   ...
   */
  async getBikesToScrape(limit = null) {
    const query = `
      SELECT 
        bd.keyid,
        bc.make,
  ...
  transformToCleanFormat(keyid, bikeData) {
    ...
    const transformed = {
      ...
      // Core identifiers
      makerid: bike.makerid || bike.manufacturer?.toLowerCase() || null,
      manufacturer: bike.manufacturer || null,
      familyid: bike.familyid || null,
      familyname: bike.familyname || bike.family?.familyname || null,
      ...
      buildkind: bike.buildkind || (bike.isebike ? 'e-bike' : 'bike'),
      gender: bike.gender || 'unisex',
      
      // URLs
      canonical_url: bike.url || bike.canonicalurl || null,
      manufacturer_url: bike.manufacturerurl || null,
      manufacturer_product_url: bike.manufacturerproducturl || null,
      primary_image_url: bike.primarythumbnailurl || bike.images?.[0]?.url || null,
      
      // Timestamps
      imported_at: bike.meta?.source?.imported || null,
      modified_at: bike.meta?.source?.modified || null,
      
      // Pricing
      msrp_cents: bike.msrp ? bike.msrp * 100 : null,
      display_price_cents: bike.displayprice?.amount ? bike.displayprice.amount * 100 : null,
      display_price_currency: bike.displayprice?.currency || 'USD',
      
      // Frame
      frame_material: bike.framematerial || null,
      frame_colors: bike.colors || [],
      weight_limit_kg: bike.weightlimit?.weightkg || null,
    ...
    // Process electric specs if e-bike
    if (bike.isebike && (bike.components?.motor || bike.components?.battery)) {
      ...
      if (bike.components?.motor?.description) {
        ...
        const motorMatch = motorDesc.match(/(\w+)\s+.*?(\d+)w\s+(\d+)nm/i);
        if (motorMatch) {
          transformed.electric_specs.motor = {
            ...
            type: motorDesc.toLowerCase().includes('mid') ? 'mid-drive' : 'hub'
          };
        }
      }
      ...
      if (bike.components?.battery?.description) {
        ...
        const batteryMatch = batteryDesc.match(/(\d+)\s*wh/i);
        if (batteryMatch) {
  ...
  /**
   * Save scraped data to both tables
   */
  async saveScrapedData(keyid, scrapedData) {
    // Save to raw scraped data table
    await this.pool.query(`
      CREATE TABLE IF NOT EXISTS scraped_bike_data (
    ...
    await this.pool.query(`
      INSERT INTO scraped_bike_data (keyid, url, has_embedded_data, extracted_data, raw_data)
      VALUES ($1, $2, $3, $4, $5)
      ON CONFLICT (keyid) DO UPDATE SET
        url = EXCLUDED.url,
        scraped_at = CURRENT_TIMESTAMP,
        has_embedded_data = EXCLUDED.has_embedded_data,
  ...
  async processAll(options = {}) {
    const { limit = null, saveToFile = true } = options;
    
    console.log(chalk.bold('\n🚀 Enhanced Bike Data Scraper v4.5\n'));
    
    // Get bikes to scrape
    const bikes = await this.getBikesToScrape(limit);
...
Usage: node 04.5_enhanced_scraper.js [options]
