...
class RawrrScraper extends BaseScraper {
  ...
  async scrape() {
    ...
    try {
      ...
      // First, get the products page - try different possible URLs
      const possiblePaths = ['/collections/all', '/products', '/shop', '/'];
      let products = [];
      
      for (const path of possiblePaths) {
        const productsUrl = `${this.baseUrl}${path}`;
        console.log(`   Trying ${productsUrl}...`);
        
        try {
          const foundProducts = await this.scrapeUrl(productsUrl, async (page) => {
        ...
        return productLinks;
          ...
          console.log(`   No products at ${productsUrl}`);
        }
      }
      
      if (products.length === 0) {
        console.log(`   ⚠️  No products found. Site may need manual inspection.`);
      }
      
      // Limit products to avoid timeout
      const maxProducts = 10; // Process only first 10 products
      const productsToScrape = products.slice(0, maxProducts);
      
      if (products.length > maxProducts) {
        console.log(`   ⚠️  Found ${products.length} products, limiting to ${maxProducts} to avoid timeout`);
      }
      
      // Scrape each product page
      for (const productPath of productsToScrape) {
