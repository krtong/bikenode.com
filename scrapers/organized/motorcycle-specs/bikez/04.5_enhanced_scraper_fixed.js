...
class EnhancedBikeScraper {
  ...
  /**
   * Generate 99spokes URL from bike catalog data
   */
  generateBikeUrl(make, model, year, variant) {
    ...
    // Create base URL
    const baseUrl = `https://99spokes.com/bikes/${cleanMake}/${year}/${cleanModel}`;
    
    return baseUrl;
  }

  /**
   * Load bike variants from bike_variants.json file
   */
  async loadVariantUrls() {
    try {
      const rawData = await fs.readFile("bike_variants.json", "utf8");
      const bikeVariants = JSON.parse(rawData);
      console.log(chalk.blue(`📂 Loaded bike variants for ${Object.keys(bikeVariants).length} maker/year combinations`));
      return bikeVariants;
    } catch (err) {
      console.error(chalk.red("✖  bike_variants.json not found - this file contains all the URLs to scrape"));
      throw err;
    }
  }

  /**
   * Get bikes that need scraping from bike_variants.json
   */
  async getBikesToScrape(limit = null, forceRescrape = false) {
    // Load all variants from bike_variants.json
    const bikeVariants = await this.loadVariantUrls();
    ...
    console.log(chalk.blue('🔍 Checking which bikes need scraping...'));
    
    // Parse the bike_variants.json structure: {makerYear: {families: [{variants: [...]}]}}
    for (const [makerYear, data] of Object.entries(bikeVariants)) {
      if (limit && processedCount >= limit) break;
      
      // Parse maker/year from key (format: "maker_year")
      const [maker, year] = makerYear.split('_');
      
      // Iterate through families and variants
      for (const family of data.families || []) {
        for (const variant of family.variants || []) {
          totalVariants++;
          if (limit && processedCount >= limit) break;
          
          // Check if this URL has already been scraped (unless force rescrape)
          if (!forceRescrape) {
            const existingResult = await this.pool.query(
              'SELECT keyid FROM bikes_data_2 WHERE url = $1',
              [variant.url]
            );
          ...
          bikesToScrape.push({
            keyid: null, // Will be generated by database
            make: maker,
            model: variant.variantId, // Use the variantId as the model identifier
            year: parseInt(year),
            variant: variant.name, // Use the full name as variant description
            url: variant.url
          });
    ...
    console.log(chalk.blue(`📊 Bike Scraping Status:`));
    console.log(`  Total variants in bike_variants.json: ${chalk.yellow(totalVariants)}`);
    console.log(`  Already scraped: ${chalk.green(alreadyScraped)}`);
    console.log(`  Remaining to scrape: ${chalk.yellow(bikesToScrape.length)}`);
    console.log(`  Will process: ${chalk.blue(limit ? Math.min(bikesToScrape.length, limit) : bikesToScrape.length)} ${forceRescrape ? '(including re-scrapes)' : ''}`);
    
    return bikesToScrape;
  }
  ...
  async createBikesTable() {
    // Create the main bikes table with clean schema (no foreign key constraints)
    await this.pool.query(`
      CREATE TABLE IF NOT EXISTS bikes (
        -- Primary identifiers
        id SERIAL PRIMARY KEY,
        keyid INTEGER UNIQUE, -- Independent keyid generation (no foreign key)
  ...
  async saveScrapedData(keyid, scrapedData) {
    ...
    try {
      ...
      // Check if this URL has already been scraped to prevent duplicates
      const existingResult = await client.query(
        'SELECT keyid, scraped_at FROM bikes_data_2 WHERE url = $1',
        [scrapedData.url]
      );
      
      if (existingResult.rows.length > 0) {
        console.log(chalk.yellow(`  ⚠️ URL ${scrapedData.url} already exists in bikes_data_2 (KeyID: ${existingResult.rows[0].keyid}, scraped: ${existingResult.rows[0].scraped_at}) - SKIPPING to prevent overwrite`));
        await client.query('ROLLBACK');
        return existingResult.rows[0].keyid; // Return existing keyid
      }
      ...
      // Generate a keyid if not provided (for bike_variants.json sources)
      let actualKeyid = keyid;
      let insertResult;
      
      if (!actualKeyid) {
        // Try to insert with auto-generated keyid, handling potential conflicts
        let maxRetries = 5;
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
          try {
            // Find the next available keyid
            const maxKeyidResult = await client.query('SELECT COALESCE(MAX(keyid), 0) + 1 as next_keyid FROM bikes_data_2');
            actualKeyid = maxKeyidResult.rows[0].next_keyid;
            
            // Try to insert with this keyid
            ...
            if (error.code === '23505' && error.constraint === 'bikes_data_2_pkey') {
              // Primary key conflict, retry with new keyid
              console.log(chalk.yellow(`  ⚠️ KeyID ${actualKeyid} conflict, retrying... (attempt ${attempt}/${maxRetries})`));
              if (attempt === maxRetries) {
                throw new Error(`Failed to generate unique keyid after ${maxRetries} attempts`);
              }
              continue;
            } else {
              throw error; // Re-throw non-keyid conflicts
            }
          }
        }
      } else {
        // Use provided keyid (from bikes_catalog)
        insertResult = await client.query(`
          INSERT INTO bikes_data_2 (keyid, url, has_embedded_data, extracted_data, raw_data)
          VALUES ($1, $2, $3, $4, $5)
          RETURNING keyid
      ...
      const insertedKeyid = insertResult.rows[0].keyid;
      console.log(chalk.green(`  ✅ Saved new scraped data to bikes_data_2 with keyid ${insertedKeyid}`));
      
      // Transform and save to clean bikes table if we have good data
      if (scrapedData.bike && scrapedData.hasEmbeddedData) {
        const cleanData = this.transformToCleanFormat(actualKeyid, scrapedData);
      ...
      // Return the actual keyid that was used
      return actualKeyid;
      
    } catch (error) {
      await client.query('ROLLBACK');
      console.log(chalk.red(`  ❌ Transaction failed for URL ${scrapedData.url}: ${error.message}`));
      throw error;
    } finally {
      client.release();
    }
  ...
  async processAll(options = {}) {
    ...
    for (const bike of bikes) {
      ...
      try {
        ...
        // Save to database and get the actual keyid used
        const actualKeyid = await this.saveScrapedData(bike.keyid, scrapedData);
        
        // Store in results using the actual keyid
        results[actualKeyid] = {
          keyid: actualKeyid,
          make: bike.make,
          model: bike.model,
        ...
        // Log what we found
        console.log(chalk.green(`  ✅ Assigned KeyID: ${actualKeyid}`));
