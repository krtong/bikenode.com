/**
 * Dynamic Classified Ad Scraper V2
 * Extracts ONLY full-resolution images, no thumbnails
 */
...
class DynamicScraper {
  ...
  async extractImages() {
    ...
    // For Craigslist, ONLY extract 1200x900 images
    if (this.domain.includes('craigslist')) {
      console.log('Craigslist detected - extracting full-size images only');
      ...
      allSlideImages.forEach(img => {
        if (img.src && img.src.includes('craigslist')) {
          if (img.src.includes('1200x900')) {
            images.push(img.src);
            largeCount++;
          } else if (img.src.includes('600x450')) {
            mediumCount++;
            // DO NOT add 600x450 - they are thumbnails!
          }
        }
      });
      
      // Report findings
      console.log(`1200x900 (full-size): ${largeCount} images`);
      console.log(`600x450 (thumbnails): ${mediumCount} images`);
      
      if (largeCount > 0) {
        console.log(`✅ Successfully extracted ${largeCount} full-size images!`);
      } else {
        console.log('❌ No full-size images found - only thumbnails detected');
        console.log('💡 The page needs interaction to load full-size images');
        console.log('   Try clicking the main gallery image first');
      }
  ...
  isThumbnail(img) {
    ...
    // Check dimensions in URL - skip anything 600x450 or smaller
  ...
  async extractAll() {
    ...
    try {
      ...
      this.data = {
        ...
        scraperVersion: '3.0-no-thumbnails'
      };
...
async function extractClassifiedAd() {
  console.log('Dynamic scraper V2 starting extraction...');
