...
// Load batch_4_brands.js
const batch4Path = path.join(__dirname, 'batch_4_brands.js');
const batch4Content = fs.readFileSync(batch4Path, 'utf8');
const batch4Match = batch4Content.match(/const batch_4_brands = (\[[\s\S]*\]);/);
const batch4Brands = eval(batch4Match[1]);
...
const newBrands = batch4Brands.filter(brand => !existingBrandIds.has(brand.brand_id));

console.log(`Found ${batch4Brands.length} brands in batch_4_brands.js`);
